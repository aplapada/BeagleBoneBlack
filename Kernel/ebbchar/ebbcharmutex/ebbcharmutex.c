...
static DEFINE_MUTEX(ebbchar_mutex);  /// A macro to declare a new mutex 
...
static ssize_t dev_write(struct file *filep, const char *buffer, size_t len, loff_t *offset){
  ...
  // copy_from_user(buffer,message,len);
  ...
  // copy_from_user(message,buffer,size_of_message);
   ...
   if(error_count == 0){
     sprintf(message, "%s(%zu letters)", message, len);   // appending received string with its length
     size_of_message = strlen(message);
     printk(KERN_INFO "EBBChar: Received %zu characters from the user\n", len);
   }else{
     printk(KERN_INFO "EBBChar: write failed\n"); 
   }
   return len;
}
